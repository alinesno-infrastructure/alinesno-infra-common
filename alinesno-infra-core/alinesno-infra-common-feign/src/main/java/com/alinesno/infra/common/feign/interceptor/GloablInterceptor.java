package com.alinesno.infra.common.feign.interceptor;
 ...
 * @param <T>
 */
public class GloablInterceptor<T> implements Interceptor<T> {

    private final static Logger log = LoggerFactory.getLogger(GloablInterceptor.class);
    ...
    public void onSuccess(T data, ForestRequest req, ForestResponse res) {
        ...
        Object result = (String)data;  // data参数是方法返回类型对应的返回数据结果,注意需要视情况修改对应的类型否则有可能出现类转型异常
    ...
    public void afterExecute(ForestRequest req, ForestResponse res) {
        ...
        int status = res.getStatusCode(); // 获取请求响应状态码
        String content = res.getContent(); // 获取请求的响应内容
        Object result = res.getResult(); // 获取方法返回类型对应的最终数据结果
    }
}
